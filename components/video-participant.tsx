...
export default function VideoParticipant({
  ...
  const { name, avatarUrl, videoUrl, isMuted, isVideoOn, backgroundColor = "bg-gray-700" } = participant
  ...
  return (
    ...
    >
      {isVideoOn && videoUrl ? (
        <Image src={videoUrl || "/placeholder.svg"} alt={`${name}'s video`} fill className="object-cover" />
      ) : (
        <Avatar
          className={`
            ${
